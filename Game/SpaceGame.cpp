...
bool kiko::SpaceGame::Initiliaze()
{
	...
	m_gameOverText = std::make_shared<Text>(m_font);
	m_gameOverText->Create(kiko::g_rend, "Game Over", kiko::Color{ 1,1,1,1 });
	
	m_funnyText = std::make_shared<Text>(m_font);
...
void kiko::SpaceGame::Update(float dt)
{
	...
	case kiko::SpaceGame::eState::Title:
		...
		m_titleText->Draw(kiko::g_rend, 400, 300);
		break;
	...
	case kiko::SpaceGame::eState::Game:
		...
		{
			m_spawnTimer = 0;
			std::unique_ptr<Enemy> enemy = std::make_unique<Enemy>(kiko::randomf(75.0f, 150.0f), kiko::Pi, kiko::Transform{ {kiko::random(kiko::g_rend.GetHeight()),kiko::g_rend.GetWidth()}, 0, 6 }, kiko::g_modMan.Get("EnemyShip.txt"));
			enemy->m_tag = "Enemy";
			enemy->m_game = this;
